...
// Utility class for handling simhash calculations and hamming distance comparisons
class SimHashUtil {
  static async simhash(message, fingerprintBits = 32) {
    ...
    for (const token of tokens) {
      ...
      let binaryHash = parseInt(hash, 16).toString(2).padStart(fingerprintBits, '0');
      let bitIdx = 0;
      for (const bit of binaryHash) {
        accumulator[bitIdx] += bit === '1' ? 1 : -1;
        bitIdx++;
      }
    }
    return accumulator.map(bit => (bit > 0 ? '1' : '0')).join('');
  }

  static hammingDistance(hash1, hash2) {
    return hash1.split('').reduce((distance, bit, i) => distance + (bit !== hash2[i] ? 1 : 0), 0);
  }
}
...
self.addEventListener('message', async (event) => {
  try {
    ...
    } else if (request.action === 'computeSimHash') {
      const content = request.content;
      const simHash = await SimHashUtil.simhash(content);
      event.source.postMessage({ hash: simHash });
    } else if (request.action === 'compareHammingDistance') {
      const { hash1, hash2 } = request;
      const distance = SimHashUtil.hammingDistance(hash1, hash2);
      event.source.postMessage({ distance });
    }
  } catch (error) {
